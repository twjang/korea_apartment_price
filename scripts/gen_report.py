...
def render_graph(apt: ApartmentId, date_from=20210101)->Tuple[str, FigureWidget]:
  ...
  el = go.Scatter(x=trades_x, y=trades_y, showlegend = False, marker={'color': 'blue', 'size': 10}, mode='markers', name='실거래')
  el_canceled = go.Scatter(x=canceled_trades_x, y=canceled_trades_y, showlegend = False, marker={'color': 'orange', 'size': 10, 'symbol': 'x'}, mode='markers', name='취소거래')
  fig.add_trace(el)
  ...
  for (date_end, price), cnt in sorted(fetched_price_date_cnt.items()):
    ...
    marker = go.Scatter(x=[date_end], y=[price], text=[f'{price}억: {cnt}개'], marker={'color':'red', 'size':4}, opacity=opacity, showlegend = False, name='')
