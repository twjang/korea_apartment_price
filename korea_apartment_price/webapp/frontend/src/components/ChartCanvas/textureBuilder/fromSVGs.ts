...
export const fromSVGs = async (svgEntries: Record<string, SVGEntry>): Promise<TextureFromSVGs>=> {
  // render SVGs to <img> tags
  const seKeys = Object.keys(svgEntries);
  const imgs = await Promise.all(seKeys.map(async (seKey)=>{
    const img = document.createElement('img')
    const svgDataUrl = `data:image/svg+xml;base64,${window.btoa(svgEntries[seKey].svg)}`;
    img.src = svgDataUrl;
  ...
  seKeys.forEach((seKey, idx)=>{ svgImages[seKey] = imgs[idx]; })


  // get sizes of svgs
  const svgSizes: Record<string, {width: number, height: number}> = {};
  seKeys.forEach((seKey, idx)=>{
    let width = svgEntries[seKey].width || imgs[idx].naturalWidth;
    let height = svgEntries[seKey].height || imgs[idx].naturalHeight;
    return svgSizes[seKey] = {width, height};
  });


  // sort svgs in the order of decreasing height
  const heightKeyPairs: [number, string][] = seKeys.map(seKey=>{ return [svgSizes[seKey].height, seKey] });
  heightKeyPairs.sort((a, b)=>{ return (b[0] - a[0]); });
  ...
  const placements: { 
    ...
    textureWidth => {
      ...
      heightKeyPairs.forEach(([height, seKey])=>{
        ...
        textureHeight = Math.max(textureHeight, placement[seKey].top + svgSizes[seKey].height);
      });
  ...
  placements.sort((a, b)=>{ return a.textureHeight * a.textureWidth - b.textureHeight * b.textureWidth; });
