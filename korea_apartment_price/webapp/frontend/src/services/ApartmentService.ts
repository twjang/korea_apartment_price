...
const search = async ({
  ...
  address: string;
  name: string;
}): Promise<BaseResponse<ApartmentId[]>> => {
...
const trades = async ({
  ...
  dateFrom?: number;
  dateTo?: number;
}): Promise<BaseResponse<TradeHistoryEntry[]>> => {
  const caller = new APICaller(API_HOST, accessToken);
  const resp = await caller.call({
    method: 'POST',
...
const rents = async ({
  ...
  dateFrom?: number;
  dateTo?: number;
}): Promise<BaseResponse<RentHistoryEntry[]>> => {
  const caller = new APICaller(API_HOST, accessToken);
  const resp = await caller.call({
    method: 'POST',
...
const orderbook = async ({
  ...
  dateFrom?: number;
  dateTo?: number;
}): Promise<BaseResponse<AggregatedOrderbookEntry[]>> => {
  const caller = new APICaller(API_HOST, accessToken);
  const resp = await caller.call({
    method: 'POST',
