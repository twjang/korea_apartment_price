...
export const getRouteElements = (): JSX.Element[] => {
  const traverse = (parentUrls: string[], cfg: PageConfig[]): JSX.Element[] => {
    ...
    const elems = cfg.map((e) => {
      ...
      return (
        <Route
          key={`route-${parentUrls.join('-')}-${e.path}`}
          path={e.path}
          element={<Frame />}
        >
          {childrenRoutes}
...
export const PageHierarchyContext = React.createContext(
  {} as PageHierarchyProp
);

export const PageHierarchyProvider: React.FC<{ children?: JSX.Element }> = ({
  children,
}) => {
  ...
  const traverse = (
    ...
    const elems = cfg.map((e) => {
      ...
      if (e.children) {
        ...
        curMatchedAsGrandParent =
          children.filter((e) => e.matchedAsParent).length > 0;
        curMatchedAsParent = children.filter((e) => e.matched).length > 0;
      }
      matchedAsParent =
        curMatchedAsGrandParent || curMatchedAsParent || matched;
      ...
      if (curPageInfo.children) {
        curPageInfo.children = curPageInfo.children.map((child) => {
          return { ...child, parent: curPageInfo };
        });
      }
