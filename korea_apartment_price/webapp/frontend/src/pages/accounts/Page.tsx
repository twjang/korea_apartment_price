...
const Page: React.FC<{}>= ()=>{
  ...
  const handleChangePageIdx = (e:React.ChangeEvent<unknown>, pageIdx: number) => {
    setCurrentPageIdx(pageIdx);
  }

  if (userList && currentPageIdx && paginated) {
    return (<MUI.Box>
      <MUI.Pagination count={Math.ceil(paginated?.num_items / paginated?.items_per_page)} page={currentPageIdx} onChange={handleChangePageIdx}
