...
import ApartmentService, { ApartmentIdWithSize } from '../../services/ApartmentService';
...
interface FavAddDialogProp {
  open: boolean
  handleClose?: (e:React.MouseEvent<HTMLButtonElement>)=>any
  handleReload?: ()=>any
};
...
const FavAddDialog: React.FC<FavAddDialogProp> = (prop: FavAddDialogProp) => {
  ...
  const handleSearch = () => {
    ...
    (async ()=>{
      const resp = await ApartmentService.search({ accessToken: authInfo.bearerToken as string, address, name})
  ...
  const searchOnChange = debounce('fav/searchApartment', ()=>{
  ...
  const handleAddFav = (apartIdWithSize: ApartmentIdWithSize) => {
    (async()=>{
      const resp = await FavoriteSerivce.add({ accessToken: authInfo.bearerToken as string, apartIdWithSize});
      if (resp.success) {
        snackbar.enqueueSnackbar(`${apartIdWithSize.name}이 관심단지로 추가되었습니다.`, {variant: 'success'})
      } else {
        snackbar.enqueueSnackbar(`${apartIdWithSize.name} 관심단지 추가가 실패했습니다.`, {variant: 'error'})
      }
    })();
    ...
    >
      ...
      <MUI.DialogContent>
        <MUI.DialogContentText
          ...
          tabIndex={-1}
        >
          <MUI.Box sx={{ display:'flex', flexDirection:'row', my: 1, width: '100%'}}>
          <MUI.TextField sx={{ mx: 1, flexGrow: 1 }} label="주소" id="outlined-size-small" size="small"  value={address} onChange={handleAddressChange}/>
          <MUI.TextField sx={{ mx: 1, flexGrow: 1 }} label="단지명" id="outlined-size-small" size="small"  value={name} onChange={handleNameChange} />
          </MUI.Box>

          <MUI.Box sx={{ display: 'flex', flexDirection: 'row', my: 0.5, height: '50vh', overflowY:'scroll'}}>
            {(inProgress) ? (<MUI.CircularProgress />) : (
              <DenseTable
                columns={columns}
                rows={
                  aptList.map((e, idx) => {
...
const Page: React.FC = () => {
  ...
  const handleDeleteClick=(id: number, name: string, size: number) => {
    simpleQuestionModal.openModal({
      ...
      body: <MUI.Typography>{`${name} ${size}평을 삭제하시겠습니까?`}</MUI.Typography>,
      choices: [
        {
          button: <span>삭제</span>,
          handler: () => {
            (async () => {
              if (authInfo.bearerToken) {
                await FavoriteSerivce.remove({ accessToken: authInfo.bearerToken, id });
                setIsLoading(true);
              }
            })();
          }
  ...
  if (favList) {
    Object.keys(favList).forEach(address => {
      ...
      const curRows = Object.keys(curNameToIdSize).map(curName => {
        ...
        return {
          entry: (
            <MUI.Box sx={{my:2, display: 'flex', flexDirection:'row', justifyContent:'space-between'}}>
            <MUI.Typography sx={{ flexGrow: 2, maxWidth:'25em', m:0, my: 1, marginRight:0.4}}>{address} <span style={{fontWeight: 'bold'}}>{curName}</span> </MUI.Typography> 
            <MUI.Box sx={{flexGrow: 1, maxWidth:`${idSizeList.length * 10}em`, textAlign: 'right'}}>
              {idSizeList.map(e=>{
                const id = e[0];
                const size = e[1];
                return (
                  <MUI.ButtonGroup  key={`${id}`} variant="contained" aria-label="outlined primary button group" sx={{marginRight: 1, marginBottom:1}}>
                  <MUI.Button variant='text' sx={{width:'2em'}} onClick={()=>{
                    navigate(`/report/${id}`);
                  }}>{size}</MUI.Button>
                  <MUI.Button onClick={()=>{handleDeleteClick(id, curName, size)}} color='warning'> <MUIIcon.Delete fontSize='small'/> </MUI.Button>
                  </MUI.ButtonGroup>
                );
              })}
            </MUI.Box>
  ...
  return (<MUI.Box>
    {(isLoading) ? (
      ...
      <MUI.Box sx={{ width: '100%' }}>
        <MUI.Fab color="primary" aria-label="add" sx={{
          position: 'fixed',
          bottom: 30,
          left: 100
        }}
    ...
    <FavAddDialog open={isAddDialogOpen} handleClose={handleAddDialogClose} handleReload={handleReload} />
  </MUI.Box>
  );
